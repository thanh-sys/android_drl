...
public class ClassesService {
	...
	public String findByUsernameConsultant2(String username) {
		return classesRepository.findByUsernameConsultant2(username);
	}

	public List<Classes> findByUsernameConsultant1(String username) {
		return classesRepository.findByUsernameConsultant1(username);
	}
	...
	public ResponseEntity<ClassesDTO> find(String name) {
		...
		return classesOptional
				.map(classes -> new ResponseEntity<>(new ClassesDTO(classes.getName(), classes.getConsultant().getId()),
						HttpStatus.OK))
...
//            System.out.println(consultant);
	...
	public ResponseEntity<ClassesDTO> update(String name, ClassesDTO classesDTO) {
		Optional<Classes> classesOptional = classesRepository.getByName(name);
		// query trong DB xem có đúng là có lớp này hay không -> nếu không return
